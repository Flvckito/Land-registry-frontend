...
function DashboardInner() {
    ...
    return (<div className="mx-auto max-w-6xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            ...
            . The full registry, transfer, and verification modules will be
            wired up next.
          </p>
        </div>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        ].map((c) => (<div key={c.label} className="rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-card)]">
      ...
      <div className="mt-10 overflow-hidden rounded-2xl border border-border bg-[image:var(--gradient-hero)] p-8 text-primary-foreground shadow-[var(--shadow-elegant)]">
        ...
        <p className="mt-2 max-w-2xl text-sm text-primary-foreground/80">
          Auth, layout, role-aware navigation and protected routes are in place.
          Next we'll build the Land Registry, Ownership Transfer, Blockchain
          Verification and Admin User Management modules — wired through a clean
          service layer that can swap from mock to real backend without UI changes.
        </p>
      </div>
    </div>);
}
