...
function Inner() {
    ...
    const selectedLand = useMemo(() => lands?.find((l) => l.id === form.landId) ?? null, [lands, form.landId]);
    ...
    return (<div className="mx-auto max-w-3xl">
      ...
      <header className="mt-3">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          The parcel will be flagged as <span className="font-medium">pending transfer</span>{" "}
          until a registry officer approves and settles it on-chain.
        </p>
      </header>

      <form onSubmit={onSubmit} noValidate className="mt-8 space-y-6 rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)] md:p-8">
        ...
        <div className="space-y-1.5">
          ...
          {lands === null ? (<div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Loader2 className="h-3.5 w-3.5 animate-spin"/> Loading parcels…
            </div>) : lands.length === 0 ? (<p className="text-sm text-muted-foreground">
              You don't have any transferable parcels.{" "}
              <Link to="/lands" className="text-primary hover:underline">
                View your registry
              </Link>
              .
            </p>) : (<Select value={form.landId} onValueChange={(v) => setForm((f) => ({ ...f, landId: v }))}>
              <SelectTrigger id="landId">
                <SelectValue placeholder="Choose a parcel…"/>
              </SelectTrigger>
              <SelectContent>
                {lands.map((l) => (<SelectItem key={l.id} value={l.id}>
        ...
        {selectedLand && (<div className="rounded-lg border border-border bg-muted/30 p-3 text-xs">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            <Input id="toOwnerName" value={form.toOwnerName} onChange={(e) => setForm((f) => ({ ...f, toOwnerName: e.target.value }))} placeholder="John Buyer"/>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="toOwnerNationalId" value={form.toOwnerNationalId} onChange={(e) => setForm((f) => ({
            ...f,
            toOwnerNationalId: e.target.value.toUpperCase(),
        }))} placeholder="CIT-7654321" className="font-mono"/>
            {errors.toOwnerNationalId && (<p className="text-xs text-destructive">{errors.toOwnerNationalId}</p>)}
          </div>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="reason">Reason / reference</Label>
          <Textarea id="reason" rows={3} value={form.reason} onChange={(e) => setForm((f) => ({ ...f, reason: e.target.value }))} placeholder="Sale agreement #2024-0142 dated 12 Apr 2026."/>
        ...
        <div className="flex flex-col-reverse items-stretch gap-3 border-t border-border pt-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button type="submit" disabled={submitting || !lands || lands.length === 0} className="bg-[image:var(--gradient-emerald)] text-primary-foreground shadow-[var(--shadow-elegant)] hover:opacity-95">
              {submitting ? (<>
...
export default TransferInitiationPage;