...
function NewLandPage() {
    return (<ProtectedRoute roles={["admin", "land_officer"]}>
      <NewLandInner />
    </ProtectedRoute>);
}
function NewLandInner() {
    ...
    return (<div className="mx-auto max-w-3xl">
      <Link to="/lands" className="inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-primary">
      ...
      <header className="mt-3">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Capture the survey details below. On submission the record is hashed
          and anchored to the LandChain ledger — the resulting transaction
          becomes the parcel's permanent proof of registration.
        </p>
      </header>

      <form onSubmit={onSubmit} noValidate className="mt-8 space-y-8 rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)] md:p-8">
        ...
        <Section title="Parcel">
          <Field label="Parcel number" htmlFor="parcelNumber" error={errors.parcelNumber} hint="Unique identifier from the cadastral record.">
            <Input id="parcelNumber" value={form.parcelNumber} onChange={(e) => set("parcelNumber", e.target.value.toUpperCase())} placeholder="KGL-CTR-001-2024" className="font-mono"/>
          </Field>
          <Field label="Title / description" htmlFor="title" error={errors.title}>
            <Input id="title" value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="Riverside plot — Kigali Centre"/>
          </Field>
        </Section>

        <Section title="Owner">
          <Field label="Owner full name" htmlFor="ownerName" error={errors.ownerName}>
            <Input id="ownerName" value={form.ownerName} onChange={(e) => set("ownerName", e.target.value)} placeholder="Jane Doe"/>
          </Field>
          <Field label="Owner national ID" htmlFor="ownerNationalId" error={errors.ownerNationalId}>
            <Input id="ownerNationalId" value={form.ownerNationalId} onChange={(e) => set("ownerNationalId", e.target.value.toUpperCase())} placeholder="CIT-1234567" className="font-mono"/>
          </Field>
        </Section>

        <Section title="Location">
          <Field label="Address / locality" htmlFor="location" error={errors.location}>
            <Input id="location" value={form.location} onChange={(e) => set("location", e.target.value)} placeholder="KN 45 St, Nyarugenge"/>
          </Field>
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="District" htmlFor="district" error={errors.district}>
              <Input id="district" value={form.district} onChange={(e) => set("district", e.target.value)} placeholder="Nyarugenge"/>
            </Field>
            <Field label="Region / Province" htmlFor="region" error={errors.region}>
              <Input id="region" value={form.region} onChange={(e) => set("region", e.target.value)} placeholder="Kigali"/>
            </Field>
          </div>
        </Section>
        ...
        <Section title="Specification">
          <div className="grid gap-5 sm:grid-cols-2">
            <Field label="Area (m²)" htmlFor="areaSqm" error={errors.areaSqm}>
              <Input id="areaSqm" type="number" inputMode="numeric" min={1} value={form.areaSqm} onChange={(e) => set("areaSqm", e.target.value)} placeholder="850"/>
            </Field>
          ...
          <Field label="Notes (optional)" htmlFor="description" error={errors.description} hint="Additional context that will be hashed alongside the record.">
            <Textarea id="description" value={form.description} onChange={(e) => set("description", e.target.value)} placeholder="Boundary stones GPS-tagged on east edge…" rows={3}/>
          </Field>
        </Section>
        ...
        <div className="flex flex-col-reverse items-stretch gap-3 border-t border-border pt-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button type="submit" disabled={submitting} className="bg-[image:var(--gradient-emerald)] text-primary-foreground shadow-[var(--shadow-elegant)] hover:opacity-95">
              {submitting ? (<>
