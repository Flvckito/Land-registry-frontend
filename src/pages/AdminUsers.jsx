...
const ROLE_TONE = {
    ...
    land_officer: "bg-[color:var(--color-gold)]/15 text-gold-foreground border-[color:var(--color-gold)]/40",
...
function Inner() {
    ...
    const filtered = useMemo(() => {
        ...
        if (!q)
            return users;
        return users.filter((u) => [u.name, u.email, u.nationalId ?? "", u.role]
            .join(" ")
            .toLowerCase()
            .includes(q));
    }, [users, query]);
    const counts = useMemo(() => {
        const base = { all: users?.length ?? 0, admin: 0, land_officer: 0, citizen: 0 };
        users?.forEach((u) => (base[u.role] += 1));
        return base;
    }, [users]);
    const onChangeRole = async (target, role) => {
        if (!me || target.role === role)
            return;
        setBusyId(target.id);
        try {
    ...
    return (<div className="mx-auto max-w-6xl">
      <header>
        ...
        <p className="mt-2 max-w-xl text-sm text-muted-foreground">
          Promote registry officers, demote inactive accounts and remove users
          who no longer need access. Role changes take effect immediately.
        </p>
      </header>
      ...
      <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="relative w-full max-w-md">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search name, email, national ID…" className="h-10 pl-9"/>
        </div>
      </div>

      <div className="mt-4 overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-card)]">
        {users === null ? (<div className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground">
            <Loader2 className="h-4 w-4 animate-spin"/> Loading users…
          </div>) : filtered.length === 0 ? (<div className="px-6 py-16 text-center text-sm text-muted-foreground">
            ...
            <TableBody>
              {filtered.map((u) => {
                ...
                return (<TableRow key={u.id} className="hover:bg-muted/30">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="truncate text-sm font-medium">
                            {u.name}
                            {isMe && (<span className="ml-2 rounded bg-muted px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                                you
                              </span>)}
                          </div>
                    ...
                    <TableCell className="py-3">
                      <span className={`inline-flex rounded-full border px-2.5 py-0.5 text-xs font-medium ${ROLE_TONE[u.role]}`}>
                        {ROLE_LABEL[u.role]}
                      </span>
                    </TableCell>
                    <TableCell className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <Select value={u.role} onValueChange={(v) => onChangeRole(u, v)} disabled={busyId === u.id || (isMe && u.role === "admin")}>
                        ...
                        <Button variant="ghost" size="icon" disabled={isMe || busyId === u.id} onClick={() => setConfirmDelete(u)} className="text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label={`Delete ${u.name}`}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmDelete?.name} will lose access immediately. Land records
              they own remain on the ledger and are unaffected.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function Stat({ label, value, icon: Icon, }) {
    return (<div className="rounded-xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
