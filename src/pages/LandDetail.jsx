...
import { ArrowLeft, Calendar, CheckCircle2, FileSignature, Loader2, MapPin, Ruler, ShieldCheck, User as UserIcon, } from "lucide-react";
...
function LandDetailInner() {
    ...
    if (land === null) {
        ...
        <Link to="/lands" className="mt-6 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline">
    ...
    return (<div className="mx-auto max-w-5xl">
      ...
      <div className="mt-3 overflow-hidden rounded-2xl border border-border bg-[image:var(--gradient-hero)] p-6 text-primary-foreground shadow-[var(--shadow-elegant)] md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <StatusBadge status={land.status} className="bg-white/10 text-primary-foreground border-white/20 backdrop-blur"/>
        </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          ...
          <div className="rounded-lg bg-white/5 px-3 py-2 text-xs">
            ...
            <div className="mt-0.5 font-mono">
              {land.anchor.blockNumber.toLocaleString()}
            </div>
          </div>
          <div className="rounded-lg bg-white/5 px-3 py-2 text-xs">
            <div className="font-semibold uppercase tracking-wider text-primary-foreground/70">
        ...
        <div className="space-y-6">
          ...
          <Card title="Chain of custody" icon={Calendar}>
            <ol className="relative space-y-5 border-l border-border pl-5">
              {land.history.map((evt) => (<li key={evt.id} className="relative">
                  <span className="absolute -left-[27px] top-0.5 grid h-5 w-5 place-items-center rounded-full bg-primary text-primary-foreground">
                  ...
                  {evt.notes && (<p className="mt-1 text-xs text-muted-foreground">
                      {evt.notes}
                    </p>)}
                  <div className="mt-2">
...
function Row({ label, value, mono, capitalize, }) {
    return (<div className="grid grid-cols-[110px_1fr] items-start gap-3 text-sm">
...
function AnchorRow({ label, value }) {
    return (<div className="rounded-lg bg-white/5 px-3 py-2 text-xs">
      <div className="font-semibold uppercase tracking-wider text-primary-foreground/70">
        {label}
      </div>
      <div className="mt-1">
        <HashChip hash={value} truncate={8} className="border-white/15 bg-white/10 text-primary-foreground hover:bg-white/15 hover:text-primary-foreground"/>
      </div>
    </div>);
}
