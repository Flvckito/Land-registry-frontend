...
function Inner() {
    ...
    return (<div className="mx-auto max-w-6xl">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            Every ownership change is anchored on initiation and again on
            settlement, producing a complete two-step audit trail.
          </p>
        </div>
        <Button asChild className="h-10 bg-[image:var(--gradient-emerald)] text-primary-foreground shadow-[var(--shadow-elegant)] hover:opacity-95">
      ...
      <div className="mt-6 flex flex-wrap gap-1.5">
        {["all", "pending", "approved", "rejected", "cancelled"].map((k) => (<button key={k} onClick={() => setFilter(k)} className={"rounded-full border px-3 py-1 text-xs font-medium capitalize transition " +
                (filter === k
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-card)]">
        {items === null ? (<div className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground">
            <Loader2 className="h-4 w-4 animate-spin"/> Loading transfers…
          </div>) : filtered.length === 0 ? (<div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
            ...
            <p className="max-w-sm text-sm text-muted-foreground">
              When you initiate or are involved in a transfer, it will appear
              here with its full chain of anchors.
            </p>
          </div>) : (<Table>
            <TableHeader>
            ...
            <TableBody>
              {filtered.map((t) => (<TableRow key={t.id} className="hover:bg-muted/30">
                  ...
                  <TableCell className="hidden py-3 xl:table-cell">
                    <HashChip hash={(t.settlementAnchor ?? t.initiationAnchor).txHash} label="tx" truncate={6}/>
                  </TableCell>
