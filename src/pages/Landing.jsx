...
function LandingPage() {
    return (<PublicLayout>
      ...
      <section className="relative isolate overflow-hidden">
        <div aria-hidden className="absolute inset-0 -z-10 bg-[image:var(--gradient-hero)]"/>
        <div aria-hidden className="absolute inset-0 -z-10 opacity-[0.07] [background-image:linear-gradient(var(--color-gold)_1px,transparent_1px),linear-gradient(90deg,var(--color-gold)_1px,transparent_1px)] [background-size:48px_48px]"/>
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-20 md:grid-cols-[1.1fr_0.9fr] md:gap-16 md:px-8 md:py-28 lg:py-32">
          <div className="text-primary-foreground">
            <div className="inline-flex items-center gap-2 rounded-full border border-[color:var(--color-gold)]/40 bg-background/5 px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] text-[color:var(--color-gold)] backdrop-blur">
            ...
            <p className="mt-6 max-w-xl text-base leading-relaxed text-primary-foreground/80 md:text-lg">
              LandChain Registry is the national platform for registering,
              transferring and verifying land titles. Every action is cryptographically
              signed and anchored to a public ledger — preventing fraud, eliminating
              disputes, and giving citizens true certainty over their property.
            </p>
            <div className="mt-9 flex flex-wrap items-center gap-3">
              <Button asChild size="lg" className="bg-[image:var(--gradient-gold)] text-gold-foreground shadow-[var(--shadow-gold)] hover:opacity-95">
                <Link to="/register">
            ...
            ["3", "Authority roles"],
                  ...
                  <dt className="font-display text-2xl font-bold text-[color:var(--color-gold)]">
                    {k}
                  </dt>
          ...
          <div className="relative">
            <div className="absolute -inset-4 -z-10 rounded-3xl bg-[color:var(--color-gold)]/10 blur-2xl"/>
            <div className="rounded-2xl border border-primary-foreground/10 bg-card/95 p-6 shadow-[var(--shadow-elegant)] backdrop-blur md:p-8">
            ...
            ["Issued", "12 Mar 2026"],
              ...
              <div className="mt-4 h-1 w-full overflow-hidden rounded-full bg-muted">
                <div className="h-full w-3/4 bg-[image:var(--gradient-gold)]"/>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-20 md:px-8">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-muted-foreground">
              Three pillars hold the platform together — authority, immutability,
              and openness — so every land record can be trusted without
              question.
            </p>
          </div>
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {features.map((f) => (<div key={f.title} className="group rounded-xl border border-border bg-card p-6 shadow-[var(--shadow-card)] transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-elegant)]">
