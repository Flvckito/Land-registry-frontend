...
import { AlertCircle, ArrowRight, CheckCircle2, FileSearch, Loader2, ShieldCheck, } from "lucide-react";
...
function VerifyPage() {
    ...
    const lookupByParcel = async () => {
        ...
        setResult(rec
            ? { kind: "found", record: rec, matchedAnchor: { type: "Registration", tx: rec.anchor.txHash } }
    ...
    return (<div className="min-h-screen bg-background">
      <header className="border-b border-border bg-[image:var(--gradient-hero)] text-primary-foreground">
        <div className="mx-auto flex max-w-5xl flex-col gap-4 px-6 py-10 md:py-14">
          <Link to="/" className="inline-flex w-fit items-center gap-1.5 text-xs font-medium text-primary-foreground/80 hover:text-primary-foreground">
            ← Home
          </Link>
          <div className="inline-flex w-fit items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[color:var(--color-gold)]">
          ...
          <p className="max-w-2xl text-sm text-primary-foreground/80 md:text-base">
            Anyone can audit a parcel without an account. Look it up by
            parcel number or by any transaction hash from its chain of
            custody — the response is signed by the LandChain ledger and
            cannot be forged.
          </p>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
          ...
          <Tabs defaultValue="parcel">
            ...
            lookupByParcel();
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <Input id="parcel" value={parcel} onChange={(e) => setParcel(e.target.value.toUpperCase())} placeholder="KGL-CTR-001-2024" className="font-mono"/>
                </div>
                <Button type="submit" disabled={!parcel.trim() || result.kind === "loading"} className="bg-[image:var(--gradient-emerald)] text-primary-foreground hover:opacity-95">
                  {result.kind === "loading" ? (<Loader2 className="h-4 w-4 animate-spin"/>) : (<FileSearch className="h-4 w-4"/>)}
                  Verify
                </Button>
              </form>
            </TabsContent>
            ...
            lookupByTx();
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <Input id="tx" value={tx} onChange={(e) => setTx(e.target.value)} placeholder="0x…" className="font-mono"/>
                </div>
                <Button type="submit" disabled={!tx.trim() || result.kind === "loading"} className="bg-[image:var(--gradient-emerald)] text-primary-foreground hover:opacity-95">
                  {result.kind === "loading" ? (<Loader2 className="h-4 w-4 animate-spin"/>) : (<FileSearch className="h-4 w-4"/>)}
                  Verify
                </Button>
              </form>
        ...
        <div className="mt-6">
          ...
          {result.kind === "loading" && (<div className="flex items-center justify-center gap-2 py-12 text-sm text-muted-foreground">
              <Loader2 className="h-4 w-4 animate-spin"/> Querying the ledger…
            </div>)}
          {result.kind === "not_found" && (<div className="flex items-start gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-5 text-sm text-destructive">
              ...
              <div>
                ...
                <p className="mt-0.5 text-destructive/80">
                  ...
                  <span className="font-mono">{result.query}</span>. Double-check
...
function Hints() {
    return (<div className="grid gap-4 sm:grid-cols-2">
      {[
        ...
        ].map((h) => (<div key={h.title} className="rounded-xl border border-border bg-card p-5 shadow-[var(--shadow-card)]">
...
function FoundCard({ record, matchedAnchor, }) {
    return (<div className="overflow-hidden rounded-2xl border border-success/40 bg-card shadow-[var(--shadow-card)]">
      ...
      <div className="grid gap-6 p-6 md:grid-cols-3">
        <div className="md:col-span-2">
          ...
          <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
            ...
            <Field label="Location" value={`${record.location}, ${record.district}, ${record.region}`}/>
...
function Field({ label, value, mono, capitalize, children, }) {
    return (<div>
      <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
        {label}
      </div>
      {children ?? (<div className={`mt-1 text-foreground ${mono ? "font-mono text-xs" : ""} ${capitalize ? "capitalize" : ""}`}>
          {value}
        </div>)}
    </div>);
}
