    ...
    return (<div className="mx-auto max-w-6xl">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            Every record below is anchored to the LandChain ledger. Click a
            parcel to view its full chain of custody.
          </p>
        </div>
        {canRegister && (<Button asChild className="h-10 bg-[image:var(--gradient-emerald)] text-primary-foreground shadow-[var(--shadow-elegant)] hover:opacity-95">
            <Link to="/lands/new">
      ...
      <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="relative w-full max-w-md">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search parcel #, owner, district…" className="h-10 pl-9"/>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {["all", "registered", "pending_transfer", "disputed"].map((key) => (<button key={key} onClick={() => setStatusFilter(key)} className={"rounded-full border px-3 py-1 text-xs font-medium capitalize transition " +
                (statusFilter === key
      ...
      <div className="mt-4 overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-card)]">
        {lands === null ? (<div className="flex items-center justify-center gap-2 py-16 text-sm text-muted-foreground">
            <Loader2 className="h-4 w-4 animate-spin"/> Loading registry…
          </div>) : filtered.length === 0 ? (<EmptyState canRegister={canRegister} hasRecords={(lands?.length ?? 0) > 0}/>) : (<Table>
            <TableHeader>
...
function EmptyState({ canRegister, hasRecords, }) {
    return (<div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
      ...
      {!hasRecords && canRegister && (<Button asChild className="mt-2 bg-[image:var(--gradient-emerald)] text-primary-foreground">
          <Link to="/lands/new">
            <Plus className="h-4 w-4"/> Register land
          </Link>
        </Button>)}
      <div className="mt-3 inline-flex items-center gap-1.5 text-xs text-muted-foreground">
        <ShieldCheck className="h-3.5 w-3.5 text-primary"/> Records are
        immutable once anchored.
      </div>
    </div>);
}
export default LandsListPage;